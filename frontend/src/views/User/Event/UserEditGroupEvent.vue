...
watch(() => props.show, async (newShow) => {
  if (newShow && props.eventId && props.id) {
    ...
    try {
      const response = await api.get(`/user/events/${props.eventId}/group-scans/${props.id}`)
...
const handleSubmit = async () => {
  ...
  try {
    await api.put(`/user/events/${props.eventId}/group-scans/${props.id}`, { name_group: GroupName.value, status: GroupStatus.value })
