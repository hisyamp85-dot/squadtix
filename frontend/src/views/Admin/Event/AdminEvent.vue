<template>
  <AdminLayout>
    ...
    >
      ...
      >
        <!-- Kalau tidak ada komponennya, boleh dihapus baris ini -->
        <ProfileCard />
          ...
          <table class="min-w-full border border-separate border-spacing-y-2">
            <thead>
              <tr class="border-b">
                ...
                <th class="px-5 py-3 text-left text-gray-900 dark:text-white">Total Category</th>
        ...
        <!-- PAGINATION (SAMA STYLE DENGAN CHECKIN LOGS) -->
        <div
...
// Pastikan entries selalu number
...
// Nomor halaman seperti di checkin logs
...
const fetchEvents = async () => {
  try {
    ...
    events.value = data.map((event) => ({
      ...
      categories: new Array(event.totalCategories).fill(""), // cuma buat .length
      status: event.status,
      id_user: event.id_user,
    }));

    // pastikan currentPage tidak out of range setelah data berubah
    if (currentPage.value > totalPages.value) {
