...
const addGroupToken = (value: string) => {
  ...
  const alreadyExists = groups.value.some(
    g => g.toLowerCase() === token.toLowerCase()
  )
  if (alreadyExists) {
...
const handleBackspaceGroup = (event: KeyboardEvent) => {
  if (newGroup.value === '' && groups.value.length > 0) {
    event.preventDefault()
    const last = groups.value.pop()
    if (last) {
      newGroup.value = last // user bisa edit, nanti auto-commit lagi
    }
  }
}
...
const saveEditing = () => {
  ...
  const exists = groups.value.some(
    (g, i) => i !== editingIndex.value && g.toLowerCase() === value.toLowerCase()
  )
  if (exists) {
    toast.warning('Group name already exists')
  } else {
